let base = new function() {
    ...
    $.ajax({
        url: '../dashbord/getTitelPrice.php',
        method: 'GET',
        dataType: 'json',
        async: false,
        success: function(response) {
    ...
    this.a = new function() {
        ...
        for (k in bA4) {
            bA4[k] = Math.ceil(Math.round(bA4[k] * z) / 100);
        }
        this.bA4 = bA4;
    ...
    this.c = new function() {
        ...
        for (k in ppr) {
            ppr[k] = Math.ceil(Math.round(ppr[k] * z) / 100);
        }
        this.price = ppr;
    ...
    this.d = new function() {
        ...
        let pr = [
            ...
            [
                prRBD["dis_foto_10x15"],
                prRBD["dis_foto_15x20"],
                prRBD["dis_foto_20x30"],
                prRBD["dis_foto_30x40"]
            ], //Печать фото
        ]
        this.price = pr;
        this.lst = new function() {
...
let chek = new function() {
    this.const = function() {
        ...
        this.cn = fio.value; //Название компании еcли это безнал
        ...
        this.telc = phone.value; //Название компании еcли это безнал
    ...
    this.fix3 = function(id) {
        this.data.fx = 2;
        idOrder = this.saveOrder();
        this.data = new this.const;
    }
    ...
    this.saveOrder = function() {
        ...
        let func = (resp) => {
            respobj = JSON.parse(resp);
            let cheksId = document.getElementsByClassName('chekId');
            this.data.id = respobj.lastId;
            $('body').attr('id', this.data.id);
            $(document).trigger('save-order-end');
    ...
    this.loadCher = function(idloadcheck2) {
        ...
        $.ajax({
            ...
            async: false,
            data: {
                idloadcheck: encodeURIComponent(idloadcheck2)
            },
            dataType: 'json',
            success: function(response) {
                objInfo = response;
...
let main = new function() {
    ...
    this.userInit = function() {
        ...
        fio.addEventListener('change', function() {
            chek.setCompName(fio.value);
        }, false);
        phone.addEventListener('change', function() {
            chek.setTel(phone.value);
        }, false);
