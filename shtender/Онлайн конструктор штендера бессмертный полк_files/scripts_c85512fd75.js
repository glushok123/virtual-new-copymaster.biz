...
(function($) {
    var $container1,
        orig_src1 = new Image(),
        image_target1 = $(".img1.resize-image"),
        event_state1 = {},
        min_width = 60,
        min_height = 60,
        max_width = 1800,
        ...
        init_height = 500,
        current_template = "01",
        resize_canvas1 = document.createElement("canvas");
    resize_canvas1.id = "canvas1";
    imageData1 = null;

    var $container2,
        orig_src2 = new Image(),
        image_target2 = $(".img2.resize-image"),
        event_state2 = {},
        resize_canvas2 = document.createElement("canvas");
    resize_canvas2.id = "canvas2";
    imageData2 = null;
    ...
    var listRU = $.masksSort(
        $.masksLoad(
            "https://cdn.rawgit.com/andr-04/inputmask-multi/master/data/phones-ru.json"
        ),
        ["#"],
        /[0-9]|#/,
        "mask"
    ...
    initImg = function(imnum) {
        if ((imnum = "img1")) {
            $(image_target1)
                .height(init_height)
                .wrap('<div class="' + imnum + ' resize-container"></div>')
                .before(
                    '<span class="' + imnum + ' resize-handle resize-handle-nw"></span>'
                )
                .before(
                    '<span class="' + imnum + ' resize-handle resize-handle-ne"></span>'
                )
                .after(
                    '<span class="' + imnum + ' resize-handle resize-handle-se"></span>'
                )
                .after(
                    '<span class="' + imnum + ' resize-handle resize-handle-sw"></span>'
                );
            $container1 = $("." + imnum + ".resize-container");
            $container1.prepend(
                '<div class="' + imnum + ' resize-container-ontop"></div>'
            );
            $container1.on("mousedown touchstart", ".resize-handle", startResize1);
            $container1.on(
        ...
        if ((imnum = "img2")) {
            ...
            $container2 = $("." + imnum + ".resize-container");
            $container2.prepend(
                '<div class="' + imnum + ' resize-container-ontop"></div>'
            );
            $container2.on("mousedown touchstart", ".resize-handle", startResize2);
            $container2.on(
                "mousedown touchstart",
                ".resize-container-ontop",
                startMoving2
    ...
    startResize1 = function(e) {
        e.preventDefault();
        e.stopPropagation();
        saveEventState1(e);
        $(document).on("mousemove touchmove", resizing1);
        $(document).on("mouseup touchend", endResize1);
    };
    startResize2 = function(e) {
        e.preventDefault();
        e.stopPropagation();
        saveEventState2(e);
        $(document).on("mousemove touchmove", resizing2);
        $(document).on("mouseup touchend", endResize2);
    };

    endResize1 = function(e) {
        resizeImageCanvas1($(image_target1).width(), $(image_target1).height());
        e.preventDefault();
        $(document).off("mouseup touchend", endResize1);
        $(document).off("mousemove touchmove", resizing1);
    };
    endResize2 = function(e) {
        resizeImageCanvas2($(image_target2).width(), $(image_target2).height());
        e.preventDefault();
        $(document).off("mouseup touchend", endResize2);
        $(document).off("mousemove touchmove", resizing2);
    };

    saveEventState1 = function(e) {
        // Save the initial event details and container state
        event_state1.container_width = $container1.width();
        event_state1.container_height = $container1.height();
        event_state1.container_left = $container1.offset().left;
        event_state1.container_top = $container1.offset().top;
        event_state1.mouse_x =
            (e.clientX || e.pageX || e.originalEvent.touches[0].clientX) +
            $(window).scrollLeft();
        event_state1.mouse_y =
        ...
        if (typeof e.originalEvent.touches !== "undefined") {
            event_state1.touches = [];
            $.each(e.originalEvent.touches, function(i, ob) {
                event_state1.touches[i] = {};
                event_state1.touches[i].clientX = 0 + ob.clientX;
                event_state1.touches[i].clientY = 0 + ob.clientY;
            });
        }
        event_state1.evnt = e;
    };
    saveEventState2 = function(e) {
        // Save the initial event details and container state
        event_state2.container_width = $container2.width();
        event_state2.container_height = $container2.height();
        event_state2.container_left = $container2.offset().left;
        event_state2.container_top = $container2.offset().top;
        event_state2.mouse_x =
            (e.clientX || e.pageX || e.originalEvent.touches[0].clientX) +
            $(window).scrollLeft();
        event_state2.mouse_y =
            (e.clientY || e.pageY || e.originalEvent.touches[0].clientY) +
            $(window).scrollTop();
        ...
        // For some reason it does not allow a direct copy of the touches property
        if (typeof e.originalEvent.touches !== "undefined") {
            event_state2.touches = [];
            $.each(e.originalEvent.touches, function(i, ob) {
                event_state2.touches[i] = {};
                event_state2.touches[i].clientX = 0 + ob.clientX;
                event_state2.touches[i].clientY = 0 + ob.clientY;
            });
        }
        event_state2.evnt = e;
    };

    resizing1 = function(e) {
        var mouse = {},
            width,
            height,
            left,
        ...
        if ($(event_state1.evnt.target).hasClass("resize-handle-se")) {
            width = mouse.x - event_state1.container_left;

            left = event_state1.container_left;
            top = event_state1.container_top;
        } else if ($(event_state1.evnt.target).hasClass("resize-handle-sw")) {
            width =
                event_state1.container_width - (mouse.x - event_state1.container_left);
            left = mouse.x;
            top = event_state1.container_top;
        } else if ($(event_state1.evnt.target).hasClass("resize-handle-nw")) {
            width =
                event_state1.container_width - (mouse.x - event_state1.container_left);
            left = mouse.x;
            top = mouse.y - ((width / orig_src1.width) * orig_src1.height - height);
        } else if ($(event_state1.evnt.target).hasClass("resize-handle-ne")) {
            width = mouse.x - event_state1.container_left;
            left = event_state1.container_left;
            top = mouse.y - ((width / orig_src1.width) * orig_src1.height - height);
        }
        height = (width / orig_src1.width) * orig_src1.height;
        if (
            width > min_width &&
            height > min_height &&
            width < max_width &&
    ...
    resizing2 = function(e) {
        ...
        if ($(event_state2.evnt.target).hasClass("resize-handle-se")) {
            width = mouse.x - event_state2.container_left;

            left = event_state2.container_left;
            top = event_state2.container_top;
        } else if ($(event_state2.evnt.target).hasClass("resize-handle-sw")) {
            width =
                event_state2.container_width - (mouse.x - event_state2.container_left);
            left = mouse.x;
            top = event_state2.container_top;
        } else if ($(event_state2.evnt.target).hasClass("resize-handle-nw")) {
            width =
                event_state2.container_width - (mouse.x - event_state2.container_left);
            left = mouse.x;
            top = mouse.y - ((width / orig_src2.width) * orig_src2.height - height);
        } else if ($(event_state2.evnt.target).hasClass("resize-handle-ne")) {
            width = mouse.x - event_state2.container_left;
            left = event_state2.container_left;
            top = mouse.y - ((width / orig_src2.width) * orig_src2.height - height);
        }
        height = (width / orig_src2.width) * orig_src2.height;
        if (
            width > min_width &&
            height > min_height &&
            width < max_width &&
    ...
    resizeImage1 = function(width, height) {
        $(image_target1)
            .width(width)
            .height(height);
    };
    resizeImage2 = function(width, height) {
        $(image_target2)
            .width(width)
            .height(height);
    };

    resizeImageCanvas1 = function(width, height) {
        resize_canvas1.width = width;
        resize_canvas1.height = height;
        resize_canvas1.getContext("2d").drawImage(orig_src1, 0, 0, width, height);
        $(image_target1).attr("src", resize_canvas1.toDataURL("image/png"));
    };
    resizeImageCanvas2 = function(width, height) {
        resize_canvas2.width = width;
        resize_canvas2.height = height;
        resize_canvas2.getContext("2d").drawImage(orig_src2, 0, 0, width, height);
        $(image_target2).attr("src", resize_canvas2.toDataURL("image/png"));
    };

    startMoving1 = function(e) {
        e.preventDefault();
        e.stopPropagation();
        saveEventState1(e);
        $(document).on("mousemove touchmove", moving1);
        $(document).on("mouseup touchend", endMoving1);
    };
    startMoving2 = function(e) {
        e.preventDefault();
        e.stopPropagation();
        saveEventState2(e);
        $(document).on("mousemove touchmove", moving2);
        $(document).on("mouseup touchend", endMoving2);
    };

    endMoving1 = function(e) {
        e.preventDefault();
        $(document).off("mouseup touchend", endMoving1);
        $(document).off("mousemove touchmove", moving1);
    };
    endMoving2 = function(e) {
        e.preventDefault();
        $(document).off("mouseup touchend", endMoving2);
        $(document).off("mousemove touchmove", moving2);
    };
    ...
    moving1 = function(e) {
        ...
        $container1.offset({
            left: mouse.x - (event_state1.mouse_x - event_state1.container_left),
            top: mouse.y - (event_state1.mouse_y - event_state1.container_top)
        });
        // Watch for pinch zoom gesture while moving
        if (
            event_state1.touches &&
            event_state1.touches.length > 1 &&
            touches.length > 1
        ) {
            var width = event_state1.container_width,
                height = event_state1.container_height;
            var a = event_state1.touches[0].clientX - event_state1.touches[1].clientX;
            a = a * a;
            var b = event_state1.touches[0].clientY - event_state1.touches[1].clientY;
            b = b * b;
    ...
    moving2 = function(e) {
        ...
        mouse.y =
            (e.clientY || e.pageY || touches[0].clientY) + $(window).scrollTop();
        $container2.offset({
            left: mouse.x - (event_state2.mouse_x - event_state2.container_left),
            top: mouse.y - (event_state2.mouse_y - event_state2.container_top)
        });
        // Watch for pinch zoom gesture while moving
        if (
            event_state2.touches &&
            event_state2.touches.length > 1 &&
            touches.length > 1
        ) {
            var width = event_state2.container_width,
                height = event_state2.container_height;
            var a = event_state2.touches[0].clientX - event_state2.touches[1].clientX;
            a = a * a;
            var b = event_state2.touches[0].clientY - event_state2.touches[1].clientY;
            b = b * b;
            var dist1 = Math.sqrt(a + b);
    ...
    function refreshall() {
        if (($(window).width() <= '400') && (topdiv != "col2")) {
            $("#col2").insertBefore("#col1");
            $("#col1").width("90%");
            $("#col2").width("90%");
            topdiv = "col2";
        } else if (($(window).width() > '400') && (topdiv != "col1")) {
            $("#col1").insertBefore("#col2");
            $("#col1").width("20%");
            $("#col2").width("70%");
            topdiv = "col1";
        }
        a2photo_val = $("input[name='formatoptions']:checked").val();
        if (a2photo_val == "a32photo") {
            $(".componentframe")
                .removeClass("onephoto")
                .addClass("twophoto");
            $("#component2").show();
            $("#controls2").show();
            format = "A3 2 фото";
            ...
            $("#controls2").hide();
            $(".componentframe")
                .removeClass("twophoto")
                .addClass("onephoto");
            format = a2photo_val;
            $("#format").html(format);
        }
        $(".crop-wrapper").height($(".crop-wrapper").width() * 1.334);
        $("#mtempl").hide();
        writeText1();
        writeText2();
    }

    $("#controls_header_form").change(function() {
        refreshall();
    });
    ...
    $(document).on("click", ".select-frame", function() {
        $(".select-frame").removeClass("active");
        $(this).addClass("active");

        $(".frame").remove();
        $("#component1 > .overlay").prepend(
            '<img class="frame img-fluid" src="./assets/templates/bp' +
            $(this).attr("id") +
            '.png" />'
        );
        $("#component2 > .overlay").prepend(
            '<img class="frame img-fluid" src="./assets/templates/bp' +
            $(this).attr("id") +
            '_r.png" />'
        );
        current_template = $(this).attr("id");
        $("#templnum").html(current_template);
        writeText1();
        writeText2();
    });

    $(".img1.fileinput").change(function(evt) {
        ...
        photo1img_ext = files[0].name
            .split(".")
            .pop()
            .toLowerCase();

        reader.onload = function(e) {
            imageData1 = reader.result;
            loadData1();
        };
        reader.readAsDataURL(files[0]);
    });

    $(".img2.fileinput").change(function(evt) {
        var files = evt.target.files;
        var reader = new FileReader();
        photo2img_ext = files[0].name
            .split(".")
            .pop()
            .toLowerCase();
        reader.onload = function(e) {
            imageData2 = reader.result;
            loadData2();
        };
        reader.readAsDataURL(files[0]);
    });

    $(".img1.js-reset").click(function() {
        if (imageData1) loadData1();
    });
    $(".img2.js-reset").click(function() {
        if (imageData2) loadData2();
    });

    loadData1 = function() {
        image_target1.src = imageData1;
        orig_src1.src = image_target1.src;
        $("#photo1img").attr("src", imageData1);
        $(orig_src1).bind("load", function() {
            var w = $("#component1").width() * 0.6;
            var h = (w / orig_src1.width) * orig_src1.height;
            var l = $("#component1").offset().left + ($("#component1").width() - w) / 2;
            var t = ($("#component1").height() - h) / 2
            resizeImageCanvas1(w, h);
            $(".img1.resize-container").offset({
                left: l,
                top: t
            });
    ...
    loadData2 = function() {
        image_target2.src = imageData2;
        orig_src2.src = image_target2.src;
        $("#photo2img").attr("src", imageData2);
        $(orig_src2).bind("load", function() {
            var w = $("#component2").width() * 0.6;
            var h = (w / orig_src2.width) * orig_src2.height;
            var l = $("#component2").offset().left + ($("#component2").width() - w) / 2;
            var t = ($("#component2").height() - h) / 2
            resizeImageCanvas2(w, h);
            $(".img2.resize-container").offset({
                left: l,
                top: t
            });
        });
    ...
    writeText1 = function() {
        $("#name1").html($(".img1.text-1-name").val());
        $("#zv1").html($(".img1.text-2-name").val());
        $("#ye1").html($(".img1.text-3-name").val());
        var textBuffer = $("#textCanvas1")[0];

        var buffer_context = textBuffer.getContext("2d");
        if (current_template != "01") {
            textBuffer.top = $(".img1.crop-wrapper").height() * 0.8;
            textBuffer.height = $(".img1.crop-wrapper").height() * 0.2;
            textBuffer.left = $(".img1.crop-wrapper").offset().left;
            textBuffer.width = $(".img1.crop-wrapper").width();

            $("#textCanvas1").css({
                top: textBuffer.top + "px"
            });

            buffer_context.font = "italic normal 30px Arial"; //"italic normal bold " + (font_size_normal * scale) + "px " + "'" + font + "'";
            buffer_context.fillStyle = "#67211d";
            buffer_context.textAlign = "center";
            buffer_context.fillText(
                $(".img1.text-1-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.25,
                textBuffer.width * 0.95
            );
            buffer_context.font = "normal 30px Arial"; //"italic normal bold " + (font_size_normal * scale) + "px " + "'" + font + "'";
            buffer_context.fillText(
                $(".img1.text-2-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.5,
                textBuffer.width * 0.95
            );
            buffer_context.fillText(
                $(".img1.text-3-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.75,
                textBuffer.width * 0.95
            );
        } else {
            textBuffer.top = $(".img1.crop-wrapper").height() * 0.44;
            textBuffer.height = $(".img1.crop-wrapper").height() * 0.56;
            textBuffer.left = $(".img1.crop-wrapper").offset().left;
            textBuffer.width = $(".img1.crop-wrapper").width();
            $("#textCanvas1").css({
                top: textBuffer.top + "px"
            ...
            buffer_context.font = "italic normal bold 35px Arial"; //"italic normal bold " + (font_size_normal * scale) + "px " + "'" + font + "'";
            buffer_context.fillStyle = "#67211d";
            buffer_context.textAlign = "center";

            var na = $(".img1.text-1-name")
                .val()
                .split(" ");

            if (typeof na[0] !== "undefined" && na[0].length) {
                buffer_context.fillText(
                    na[0],
                    textBuffer.width / 2,
                    textBuffer.height * 0.25,
            ...
            if (typeof na[1] !== "undefined" && na[1].length) {
                buffer_context.fillText(
                    na[1],
                    textBuffer.width / 2,
                    textBuffer.height * 0.35,
                    textBuffer.width * 0.95
                );
            }
            if (typeof na[2] !== "undefined" && na[2].length) {
                buffer_context.fillText(
                    na[2],
                    textBuffer.width / 2,
                    textBuffer.height * 0.45,
                    textBuffer.width * 0.95
                );
            }
            buffer_context.font = "normal 30px Arial"; //"italic normal bold " + (font_size_normal * scale) + "px " + "'" + font + "'";
            buffer_context.fillText(
                $(".img1.text-2-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.6,
                textBuffer.width * 0.95
            );
            buffer_context.fillText(
                $(".img1.text-3-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.7,
                textBuffer.width * 0.95
            );
        }
    };
    ...
    writeText2 = function() {
        $("#name2").html($(".img2.text-1-name").val());
        $("#zv2").html($(".img2.text-2-name").val());
        $("#ye2").html($(".img2.text-3-name").val());
        var textBuffer = $("#textCanvas2")[0];

        var buffer_context = textBuffer.getContext("2d");
        if (current_template != "01") {
            textBuffer.top = $(".img2.crop-wrapper").height() * 0.8;
            textBuffer.height = $(".img2.crop-wrapper").height() * 0.2;
            textBuffer.left = $(".img2.crop-wrapper").offset().left;
            textBuffer.width = $(".img2.crop-wrapper").width();
            ...
            buffer_context.font = "normal 30px Arial"; //"italic normal bold " + (font_size_normal * scale) + "px " + "'" + font + "'";
            buffer_context.fillText(
                $(".img2.text-2-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.5,
                textBuffer.width * 0.95
            );
            buffer_context.fillText(
                $(".img2.text-3-name").val(),
                textBuffer.width / 2,
                textBuffer.height * 0.75,
                textBuffer.width * 0.95
            );
        } else {
            textBuffer.top = $(".img2.crop-wrapper").height() * 0.44;
            textBuffer.height = $(".img2.crop-wrapper").height() * 0.56;
            textBuffer.left = $(".img2.crop-wrapper").offset().left;
            textBuffer.width = $(".img2.crop-wrapper").width();
            $("#textCanvas2").css({
                top: textBuffer.top + "px"
    ...
    $(".img1.form-control").on("input", function(e) {
        var value = $(this).val();
    ...
    $(".img2.form-control").on("input", function(e) {
        var value = $(this).val();
        if ($(this).data("lastval") != value) {
            $(this).data("lastval", value);
            clearTimeout(timerid);
    ...
    $("#clientPhone").on("input", function(e) {
        if ($('#clientPhone').inputmask("isComplete")) {
            $('#btnsubmit').removeClass('disabled').prop('disabled', false).attr("rel", null);
        } else {
            $('#btnsubmit').addClass('disabled').prop('disabled', true).attr("rel", "tooltip");
        }
    });

    $("#submitForm").on("show.bs.modal", function() {
        html2canvas(document.getElementById("cropData"), {
            logging: false,
            allowTaint: false,
            useCORS: true
    ...
    function submitForm() {
        ...
        var name1 = $(".img1.text-1-name").val();
        var zv1 = $(".img1.text-2-name").val();
        var ye1 = $(".img1.text-3-name").val();
        var name2 = $(".img2.text-1-name").val();
        var zv2 = $(".img2.text-2-name").val();
        var ye2 = $(".img2.text-3-name").val();
        var formData = new FormData();
        formData.append("id", "штендер");
        formData.append("name", name);
        formData.append("email", email);
        formData.append("phone", phone);

        formData.append("name1", name1);
        formData.append("zv1", zv1);
        formData.append("ye1", ye1);
        formData.append("name2", name2);
        formData.append("zv2", zv2);
        formData.append("ye2", ye2);
        formData.append("flret", flret);
        formData.append("format", format);
        formData.append("template", current_template);
        formData.append("screenimg_ext", screenimg_ext);
        formData.append("photo1img_ext", photo1img_ext);
        formData.append("photo2img_ext", photo2img_ext);
        formData.append("screenimg", screenBlob);
        formData.append("photo1img", imageData1);
        formData.append("photo2img", imageData2);
    ...
    var imageSrcs = [
        "./assets/templates/bp01.png",
        "./assets/templates/bp01_r.png",
        "./assets/templates/bp02.png",
        "./assets/templates/bp02_r.png",
        "./assets/templates/bp03.png",
        "./assets/templates/bp03_r.png",
        "./assets/templates/bp04.png",
        "./assets/templates/bp04_r.png",
        "./assets/templates/bp05.png",
        "./assets/templates/bp05_r.png",
        "./assets/templates/bp06.png",
        "./assets/templates/bp06_r.png",
        "./assets/templates/bp07.png",
        "./assets/templates/bp07_r.png",
        "./assets/templates/bp08.png",
        "./assets/templates/bp08_r.png",
        "./assets/templates/bp09.png",
        "./assets/templates/bp09_r.png",
        "./assets/templates/bp10.png",
        "./assets/templates/bp10_r.png"
    ];
    preload(imageSrcs);
    refreshall();
    initImg(".img1");
    ...
    $('.preloader-wrapper').fadeOut();
    $('body').removeClass('preloader-site');
    $('.preloader-wrapper').css("opacity: 0.4");
