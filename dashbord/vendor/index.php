	...
	<div class="wrapper ">
		<div class="sidebar-wrapper" data-simplebar="true">
			<div class="sidebar-header">
				<div>
					<h4 class="logo-text">ОБРАБОТКА ЗАЯВОК</h4>
				</div>
			...
			<ul class="metismenu" id="menu">
				...
				<li>
					<a href="calc/calc.php">
						<div class="menu-title">Калькулятор</div>
					</a>
				</li>
				<li>
					<a href="index0.php">
						<div class="menu-title">Табель времени</div>
					</a>
				</li>
				<li>
												...
												<div class="card-body">
													...
													<div class="table-responsive">
														<div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
															...
															<thead>
																<tr role="row">
																	<th class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-sort="ascending" aria-label="id: activate to sort column descending" width: 35px;>id</th>
																	...
																	<th class="sorting ogrsize" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="comment: activate to sort column ascending"  width: 80px>Комментарий</th>
																	<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="created_at: activate to sort column ascending" style="width: 59px;">Дата создания <br> - Дата обновления</th>
...
$(document).ready(function() {
	...
	$.ajax({url: 'api/requests/', method: 'GET', dataType: 'json', success: function(response){
		...
		obj.forEach(function(entry){
			...
			{
					$("tbody").append('	<tr role="row" class="odd"><td class="sorting_1">'+entry.id+'</td><td>'+entry.tip+'</td><td>'+entry.price+'</td><td>'+entry.name+'<hr> <span style="color:;">'+entry.email+'</span><hr><span style="color:;"> '+entry.phon+'</span></td><td>'+entry.created_at+' <br>- '+entry.updated_at+'</td><td><button type="button" class="btn btn-light radius-30" data-zaiv="'+entry.id+'" data-tip="'+entry.tip+'" id="obrabzaiavka" >Обработать </button><br><br><button type="button" data-zaiv="'+entry.id+'"  class="btn btn-light radius-30  delzaiavka" >Удалить </button></td></tr>');
			}
			else{
					$("tbody").append('	<tr role="row" class="odd"><td class="sorting_1">'+entry.id+'</td><td>'+entry.tip+'</td><td>'+entry.price+'</td><td>'+entry.name+'<hr> <span style="color:;">'+entry.email+'</span><hr><span style="color:;"> '+entry.phon+'</span></td><td>'+entry.status+'</td><td>'+entry.comment+'</td><td>'+entry.created_at+' <br>- '+entry.updated_at+'</td><td><button type="button" class="btn btn-light radius-30" data-zaiv="'+entry.id+'" data-tip="'+entry.tip+'" id="obrabzaiavka">Обработать </button><br><br><button data-zaiv="'+entry.id+'" type="button" class="btn btn-light radius-30  delzaiavka" >Удалить </button></td></tr>');
			}
		  ...
		  $.ajax({url: 'api/requests/', method: 'GET', dataType: 'json', success: function(response){
			  ...
			  obj.forEach(function(entry){
				  ...
				  if (arrID.indexOf(entry.id) != -1){
					  ...
					  //console.log(entry.id+" - нету22");
					  arrID.push(entry.id);
					  console.log(entry.id+" - нету");
					  if (entry.status == "Resolved")
					  {
							  $("tbody").append('	<tr role="row" class="odd"><td class="sorting_1">'+entry.id+'</td><td>'+entry.tip+'</td><td>'+entry.price+'</td><td>'+entry.name+'</td><td>'+entry.email+'</td><td>'+entry.phon+'</td><td>'+entry.status+'</td><td>'+entry.comment+'</td><td>'+entry.created_at+'</td><td>'+entry.updated_at+'</td><td><button type="button" class="btn btn-light radius-30" data-zaiv="'+entry.id+'" data-tip="'+entry.tip+'" id="obrabzaiavka" >Обработать заявку</button><button type="button" data-zaiv="'+entry.id+'"  class="btn btn-light radius-30  delzaiavka" >Удалить заявку</button></td></tr>');
					  }
					  else{
